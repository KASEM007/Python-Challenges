...
# To solve this question, I use the .items method for dictionaries.
# Remember, the key in our nominated dictionary is a list of nominated
# directors. Think Compound Data Structures!
# I know I need to create a dictionary where the key is a director and
# the value is the number of nominations.
# But to get each director as a key, I will have to use two for loops.
# First, to iterate through the nominated dictionary's value (which here
# is a list of nominations) .
# But I have do to this again to iterate through each element
# (what I'm trying to get to - a nominated director) in the nominated list.
# After that, if the director isn't yet in our dictionary, we give that director
# a count of one. If the director is in the dictionary, we increment that
# director's count by one.
...
print(f"nom_count_dict = {nom_count_dict}")


### 1B: Create dictionary with the count of Oscar wins for each director:


# We could use the same approach as in question 1a and it would work fine,
#  but I've provided a shorter alternative here. Instead of the last 4
# lines as above, I've just written 1 line, by using the .get method.
# In this line, we find the director in the win_count_dict dictionary
# and return the value for that director (the number of times they've won).
# If they aren't in the dictionary, get returns 0 for that director.
# Then we increment that director's count by one.
